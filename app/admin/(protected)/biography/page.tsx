...
export default function AdminBiographyPage() {
  ...
  async function handleSave() {
    setSaving(true);
    if (bio) {
      await supabase.from("biography").update({ content, updated_at: new Date().toISOString() }).eq("id", bio.id);
    } else {
      const { data } = await supabase.from("biography").insert([{ content }]).select().single();
      setBio(data);
    }
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">Edit the biography displayed on the website</p>
      </div>
